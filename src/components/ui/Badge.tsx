...
export function PriceBadge({ priceType, currency }: { priceType: string | null; currency?: string | null }) {
  if (priceType === 'gift') {
    return <span className="inline-block text-xs px-2 py-0.5 rounded-full font-medium bg-purple-100 text-purple-700">贈品</span>
  }
  if (priceType === 'split') {
    return <span className="inline-block text-xs px-2 py-0.5 rounded-full font-medium bg-sky-100 text-sky-700">組合價</span>
  }
  if (currency) {
    return <span className="inline-block text-xs px-2 py-0.5 rounded-full font-medium bg-amber-100 text-amber-700">{currency}</span>
  }
  return null
}

...
const schema = z
  ...
  .refine((d) => !!(d.name_zh?.trim() || d.name_en?.trim()), {
    message: "品名中文或原文至少填一個",
    path: ["name_zh"],
  });
          ...
          <div className="space-y-1">
            <div className="relative">
              <Input
                type="number"
                {...register("price")}
                placeholder={priceType === "split" ? "分攤金額" : "選填"}
              />
              <button
                type="button"
              ...
              <div className="bg-[var(--color-bg-muted)] rounded-xl p-3 space-y-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input
                    ...
                    placeholder="匯率"
                    className="w-16 px-2 py-2 rounded-xl border border-[var(--color-border)] text-sm bg-[var(--color-bg-card)] text-[var(--color-text)] focus:outline-none"
                  />
                  <button
                    type="button"
                    disabled={!foreignAmt || !fxRate}
                    onClick={() => {
                      setValue(
                        "price",
                        Math.round(Number(foreignAmt) * Number(fxRate)),
                      );
                ...
                {foreignAmt && fxRate && (
                  <p className="text-xs text-[var(--color-primary)] font-medium">
                    ≈ NT${" "}
                    {Math.round(
                      Number(foreignAmt) * Number(fxRate),
                    ).toLocaleString()}
                  </p>
                )}
              </div>
...
export default function WishlistPage() {
  ...
  const [tab, setTab] = useState<"all" | "pending" | "purchased" | "favorites">(
    "pending",
  );
  const [typeFilter, setTypeFilter] = useState<"all" | "makeup" | "skincare">(
    "all",
  );
  ...
  async function togglePurchased(item: WishlistItem) {
    ...
    setItems((prev) =>
      prev.map((i) =>
        i.id === item.id ? { ...i, is_purchased: newPurchased } : i,
      ),
    );
  ...
  const filtered = items.filter((i) => {
    if (typeFilter !== "all" && (i.item_type ?? "makeup") !== typeFilter)
      return false;
    ...
    <div>
      ...
      {showAddForm && (
        <div className="bg-[var(--color-bg-muted)] rounded-2xl p-4 mb-5">
          <p className="text-sm font-semibold text-[var(--color-text)] mb-4">
            新增品項
          </p>
          <WishForm
            onSubmit={handleAdd}
      ...
      <div className="flex gap-2 mb-4">
        {tabs.map(({ key, label, count }) => (
          ...
          >
            {label}
            <span
              className={`text-xs px-1.5 py-0.5 rounded-full ${tab === key ? "bg-white/25" : "bg-[var(--color-border)]"}`}
            >
              {count}
            </span>
          </button>
      ...
      {tab === "favorites" &&
        ...
        (favorites.length === 0 ? (
          <EmptyState
            Icon={Heart}
            title="還沒有最愛品項"
            description="在品項詳情頁按 ♡ 加入最愛"
          />
        ) : (
          <div className="space-y-2">
            {favorites.map((fav) => {
              const name =
                [fav.name_en, fav.name_zh].filter(Boolean).join(" / ") ||
                "（未命名）";
        ...
        tab !== "favorites" && (
          <div className="space-y-3">
            {filtered.map((item) => {
              ...
              const subName =
                item.name_zh && item.name_en ? item.name_en : null;
                  ...
                  {isEditing ? (
                    <div className="p-4">
                      ...
                      <WishForm
                        ...
                        onSubmit={(data, imageFile, imageUrl) =>
                          handleEdit(item.id, data, imageFile, imageUrl)
                        }
                      ...
                      >
                        <div className="flex items-start gap-3">
                          <div className="flex-1 min-w-0">
                            ...
                            <p
                              className={`text-sm font-medium text-[var(--color-text)] ${item.is_purchased ? "line-through" : ""}`}
                            >
                              {displayName}
                            </p>
                            {subName && (
